...
class AirlockCalculateStress: XCTestCase {
    ...
    override func setUp() {
        ...
        var contextFileJSON: String = String()
        
        do {
            contextFileJSON = try String(contentsOfFile: contextFilePath)
        } catch {
            continueAfterFailure = false
            XCTFail("Unable to load context file")
        }
        ...
        for i in 1...100 {
            ...
            do {
                ...
                //try airlock.calculateFeatures(deviceContextJSON: "{}")
    ...
    func testLastSyncTime(){
        ...
        let rDate:NSDate = airlock.getLastSyncTime()
        print("Expected to be updated before \(lastSyncDate), and updated at \(rDate)")
        
        XCTAssertTrue(lastSyncDate.compare(rDate as Date) == ComparisonResult.orderedDescending, "Expected to be synced before \(lastSyncDate), but was reportedly synced at \(rDate)")
