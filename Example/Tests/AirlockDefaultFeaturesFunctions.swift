//
//  AirlockDefaultFeaturesFunctions.swift
//  AirLockSDK
//
...
class AirlockDefaultFeaturesFunctions: XCTestCase {
...
// ------------------- headsup namespace ------------------------------------------//
 ...
 // ------------------- not existing feature------------------------------------------//
    ...
    func testGetLastPullTime(){
        ...
        let rDate:NSDate = airlock.getLastPullTime()
        print("Expected to be pulled before \(lastPullDate), and pulled at \(rDate)")
        
        XCTAssertTrue(lastPullDate.compare(rDate as Date) == ComparisonResult.orderedDescending, "Expected to be pulled after \(lastPullDate), but was reportedly pulled at \(rDate)")
    }
