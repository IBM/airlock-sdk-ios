...
class GetStringTestsFr: XCTestCase {
    ...
    func testExample() {
        ...
        XCTAssertNil(airlock.getString(stringKey: "NOT EXISTING BEFORE PULL", params: "test1")) //not existing key, should return nil
        
        guard loadProduct() else {
            return
        }

        XCTAssertEqual("test1 EN VUE", airlock.getString(stringKey: "HeadsUp.InView", params: "test1")) //one param to replace
        XCTAssertEqual("DÉGAGÉ", airlock.getString(stringKey: "HeadsUp.Clear")) // no params
        XCTAssertEqual("DÉGAGÉ", airlock.getString(stringKey: "HeadsUp.Clear", params: "test1","test2","test3")) //Unused params
    ...
    private func loadProduct() -> Bool {
        ...
        let defaultFileRemotePath = TestUtils.readProductDefaultFileURL(testBundle: testBundle, name: "GetStringFunction")
