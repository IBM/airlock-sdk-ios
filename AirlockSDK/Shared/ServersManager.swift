...
internal class ServersManager {
    ...
    internal func setServer(serverName:String, product:[String:AnyObject?], onCompletion:@escaping (_ success:Bool, _ error:Error?)-> Void) {
        ...
        guard let serverInfo = self.serversInfo[serverName] else {
            onCompletion(false, nil)
            return
        }
        
        let serverURL = serverInfo.cdnOverride
        Airlock.sharedInstance.dataFethcher.retrieveDefaultsFile(serverURL: serverURL, productId: productId, seasonId: seasonId, onCompletion: { defaultsDict, err in
            guard err == nil else {
            ...
            do {
                ...
                let jsonData = try JSONSerialization.data(withJSONObject: defaultsDict as Any, options: .prettyPrinted)
    ...
    internal func retrieveServers(onCompletion:@escaping (_ servers:[String]?, _ defaultServerName:String?, _ error:Error?)-> Void) {
        ...
        Airlock.sharedInstance.dataFethcher.retrieveServersList(onCompletion: { serversArr, defaultSrv, err in
    ...
    internal func retrieveBranches(productId:String, seasonId:String, onCompletion:@escaping (_ branches:[[String:AnyObject?]]?, _ error:Error?)-> Void) {
        ...
        Airlock.sharedInstance.dataFethcher.retrieveBranchesFromServer(forSeason: true, productId: productId, seasonId: seasonId, onCompletion: { branchesArr,status,err in
            
            if (status != 200) {
                
                Airlock.sharedInstance.dataFethcher.retrieveBranchesFromServer(forSeason:false, productId: productId, seasonId: seasonId, onCompletion: { branchesArr,status,err in
                    guard err == nil else {
                        onCompletion(nil, err)
                        return
                    }
                    
                    guard let nonNullBranchesArr = branchesArr else {
