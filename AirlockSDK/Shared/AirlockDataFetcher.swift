...
internal class AirlockDataFetcher {
    ...
    private let pollsManager                            :PollsManager
    ...
    private var pollsFileModificationTime               :String?
    ...
    private let pollsFileModificationTimeQueue          = DispatchQueue(label:"PollsFileModificationTimeQueue")
    ...
    init(serversMgr: ServersManager, pollsManager: PollsManager){
        
        self.serversMgr = serversMgr
        self.pollsManager = pollsManager
        ...
        pollsFileModificationTime = UserDefaults.standard.object(forKey: POLLS_FILE_MODIFICATION_TIME_KEY) as? String ?? nil
        ...
        let branchData = AirlockFileManager.readData(BRANCH_FILE_NAME_KEY)
        ...
        self.translationsMap = AirlockFileManager.readStringDictionary(TRANSLATION_FILE_NAME_KEY)
    ...
    internal func clear() {
        ...
        _ = AirlockFileManager.removeFile(RUNTIME_FILE_NAME_KEY)
        _ = AirlockFileManager.removeFile(TRANSLATION_FILE_NAME_KEY)
        _ = AirlockFileManager.removeFile(JS_UTILS_FILE_NAME_KEY)
        _ = AirlockFileManager.removeFile(BRANCH_FILE_NAME_KEY)
        
        _ = AirlockFileManager.removeFile(STREAMS_RUNTIME_FILE_NAME_KEY)
        _ = AirlockFileManager.removeFile(STREAMS_JS_UTILS_FILE_NAME_KEY)

        _ = AirlockFileManager.removeFile(NOTIFS_RUNTIME_FILE_NAME_KEY)
        _ = AirlockFileManager.removeFile(POLLS_RUNTIME_FILE_NAME_KEY)
        
        currentBranchDict = nil
        
        self.clearModificationTimes()
        
        self.pollsManager.resultsManager.clearAll()
    }
    ...
    internal func clearModificationTimes() {
        ...
        UserDefaults.standard.removeObject(forKey:POLLS_FILE_MODIFICATION_TIME_KEY)
        pollsFileModificationTime = nil
    ...
    internal func clearPollResults() {
        self.pollsManager.resultsManager.clearAll()
    }
    ...
    internal func fetchPollResults(pollUniqueId: String, onCompletion: @escaping (_ result: AnyObject?, _ error: Error?) -> Void) {
        
        if let cachedResults = self.pollsManager.resultsManager.getPollResultsFromCache(pollUniqieId: pollUniqueId) {
            onCompletion(cachedResults, nil)
            return
        }
        
        guard let resultsUrl = self.getPollResultsURL(pollUniqueId: pollUniqueId) else {
            onCompletion(nil, AirlockError.InvalidUrl(message: "Invalid poll results URL"))
            return
        }
        
        var headers: HTTPHeaders = [:]
        
        if let cachedLastModified = self.pollsManager.resultsManager.getPollRuntimeLastModifiedDate(pollUniqieId: pollUniqueId) {
            headers = [ "If-Modified-Since": cachedLastModified ]
        }
        
        self.afManager.request(resultsUrl, method:.get, headers: headers)
            .responseData(queue: DispatchQueue.global(qos: .default)) { response in
                
                guard let responseData = response.data,
                      self.validateResponse(response: response) else {

                    onCompletion(nil, nil)
                    return
                }
                
                let responseDictionary = Utils.convertDataToJSON(data: responseData)
                onCompletion(responseDictionary, nil)
                
                let lastModified = response.response?.allHeaderFields["Date"] as? String
                self.pollsManager.resultsManager.setResults(pollUniqieId: pollUniqueId, resultsData: responseData, lastModified: lastModified)
    ...
    internal func pullDataFromServer(featuresCacheManager:FeaturesCacheManager, forcePull:Bool = false, onCompletion:@escaping (_ sucess:Bool, _ error:Error?) -> Void){
        ...
        var pollsHeaders: HTTPHeaders = [:]
        if let nonNullPollsFileModificationTime = self.pollsFileModificationTime, !forcePull {
            pollsHeaders = [ "If-Modified-Since": nonNullPollsFileModificationTime ]
        }
        ...
        // Downloading the polls runtime file (json)
        self.afManager.request(pollsURL, method:.get, headers: pollsHeaders)
            .responseData(queue: DispatchQueue.global(qos: .default)) { response in
                
                pollsTask.setResult(result: response as Any?)
            }
        tasksMgr.appendTask(task: pollsTask)
        ...
        tasksMgr.waitForTasks(onCompletion:{
            ...
            guard let pollsResponse = pollsTask.result as? AFDataResponse<Data> else {
                onCompletion(false, nil)
                return
            }
            ...
            if isOverridingBranch {
                ...
                if (!self.validateResponse(response: branchResponse)){
                    onCompletion(false, AirlockError.InvalidServerResponse(message: "Invalid response recieved from server while downloading the branch file: \(String(describing: branchResponse.response?.statusCode))"))
            ...
            if (!self.validateResponse(response: featuresResponse)){
                ...
                onCompletion(false, AirlockError.InvalidServerResponse(message: "Invalid response recieved from server while downloading the runtime file: \(String(describing: featuresResponse.response?.statusCode))"))
                return
                
            }
            
            if (!self.validateResponse(response: pollsResponse)){
                
                onCompletion(false, AirlockError.InvalidServerResponse(message: "Invalid response recieved from server while downloading the polls file: \(String(describing: featuresResponse.response?.statusCode))"))
                return
                
            }
            
            if (!self.validateResponse(response: translationsResponse)){
                onCompletion(false, AirlockError.InvalidServerResponse(message: "Invalid response recieved from server while downloading the translations file: \(String(describing: translationsResponse.response?.statusCode))"))
                return
                
            }
            
            if (!self.validateResponse(response: jsUtilsResponse, allowForEmptyData:true)) {
                onCompletion(false, AirlockError.InvalidServerResponse(message: "Invalid response recieved from server while downloading the js utils file: \(String(describing: jsUtilsResponse.response?.statusCode))"))
                return
            }
            ...
            if validateStreams {
                if !streamRunTimeResponseSuccess {
                    onCompletion(false, AirlockError.InvalidServerResponse(message: "Invalid response recieved from server while downloading the streams runtime file: \(String(describing: streamsRunTimeResponse.response?.statusCode))"))
                    return
                } else if !streamsJSUtilsResponseSuccess {
                    onCompletion(false, AirlockError.InvalidServerResponse(message: "Invalid response recieved from server while downloading the streams js utils file: \(String(describing: streamsJSUtilsResponse.response?.statusCode))"))
                    return
                }
            } else {
            ...
            if !notifictionsRunTimeResponseSuccess && validateNotifications {
                onCompletion(false, AirlockError.InvalidServerResponse(message: "Invalid response recieved from server while downloading the notifications runtime file: \(String(describing: notificationsRunTimeResponse.response?.statusCode))"))
                return
            }
            ...
            if (featuresResponse.response?.statusCode == 200){
                ...
                Airlock.sharedInstance.calculationPullQueue.sync {
                    ...
                    if let runTimeFeatures = featuresCacheManager.runTimeFeatures {
            ...
            if (pollsResponse.response?.statusCode == 200){
                
                guard let pollsData = pollsResponse.data, Airlock.sharedInstance.polls.load(pollsData: pollsData) else {
                    onCompletion(false, nil)
                    return
                }
                
                self.pollsFileModificationTimeQueue.sync {
                    self.pollsFileModificationTime = pollsResponse.response?.allHeaderFields["Date"] as! String?
                }
                UserDefaults.standard.set(self.pollsFileModificationTime, forKey:POLLS_FILE_MODIFICATION_TIME_KEY)
                
                if let pollsRuntimeResponseData = pollsResponse.data {
                    AirlockFileManager.writeData(data: pollsRuntimeResponseData, fileName: POLLS_RUNTIME_FILE_NAME_KEY)
                }
            }
            ...
            if (translationsResponse.response?.statusCode == 200){
                ...
                AirlockFileManager.writeStringDictionary(dict: stringsMap, fileName: TRANSLATION_FILE_NAME_KEY)
            ...
            if (jsUtilsResponse.response?.statusCode == 200){
                ...
                if let data = jsUtilsResponse.data {
                    
                    AirlockFileManager.writeData(data: data, fileName: JS_UTILS_FILE_NAME_KEY)
            ...
            if (streamsRunTimeResponse.response?.statusCode == 200) {
                ...
                if let streamsRunTimeResponseData = streamsRunTimeResponse.data {
                    AirlockFileManager.writeData(data: streamsRunTimeResponseData, fileName: STREAMS_RUNTIME_FILE_NAME_KEY)
                }
            }
            ...
            if (streamsJSUtilsResponse.response?.statusCode == 200){
                ...
                // Convert the response data to string
                if let data = streamsJSUtilsResponse.data {
                    AirlockFileManager.writeData(data: data, fileName: STREAMS_JS_UTILS_FILE_NAME_KEY)
                }
            ...
            if (notificationsRunTimeResponse.response?.statusCode == 200) {
                ...
                if let notificationsRunTimeResponseData = notificationsRunTimeResponse.data {
                    AirlockFileManager.writeData(data: notificationsRunTimeResponseData, fileName: NOTIFS_RUNTIME_FILE_NAME_KEY)
                }
            ...
            if let branchResponse = branchTask.result as? AFDataResponse<Data> {
                ...
                if (branchResponse.response?.statusCode == 200){
                    ...
                    if let branchResponseData = branchResponse.data {
                        AirlockFileManager.writeData(data: branchResponseData, fileName: BRANCH_FILE_NAME_KEY)
                    }
    ...
    internal func getTranslationsDict() -> [String:String]? {
        return AirlockFileManager.readStringDictionary(TRANSLATION_FILE_NAME_KEY)
    }
    
    internal func getJSUtilsString() -> String? {
        return AirlockFileManager.readString(JS_UTILS_FILE_NAME_KEY)
    }
    ...
    private func getPollsURL() -> String {
        
        guard let productConfig = self.serversMgr.activeProduct else {
            return ""
        }
        
        let runtimePath: String = "seasons/\(productConfig.productId)/\(productConfig.seasonId)/\(SERVER_POLLS_FILE_NAME)\(self.runtimeFileSuffix)\(JSON_SUFFIX)"
    ...
    private func getPollResultsURL(pollUniqueId: String) -> String? {
        
        var runtimeSuffix = SERVER_PROD_SUFFIX
        
        if Airlock.sharedInstance.isDevUser() == true {
            runtimeSuffix = SERVER_DEV_SUFFIX
        }
        ...
        let runtimePath: String = "seasons/\(productConfig.productId)/\(productConfig.seasonId)/pollResults/\(pollUniqueId)/\(SERVER_POLL_RESULTS_FILE_NAME)\(runtimeSuffix)\(JSON_SUFFIX)"
