...
class JSClientEngine {
    ...
    func calculateFeatures(runTimeFeatures:FeaturesCache,results:inout FeaturesCache,errorInfo:inout [JSErrorInfo]) {
        let rootResult = Feature(type:Type.ROOT,uniqueId:"",name:Feature.ROOT_NAME,source:.SERVER)
        ...
        var resultsProtocol = results as FeaturesMangement
        _ = calculateFeatursTree(parent:root,results:&resultsProtocol,mutexConstraint:-1,errorInfo:&errorInfo)
    }

    func calculateEntitlements(runTimeFeatures:FeaturesCache,results:inout FeaturesCache,errorInfo:inout [JSErrorInfo]) {
        let rootResult = Entitlement(type:Type.ROOT,uniqueId:"",name:Feature.ROOT_NAME,source:.SERVER)
        rootResult.isFeatureOn = true
        ...
        var resultsProtocol = results.entitlements as FeaturesMangement
            ...
            if childResult.isOn() {
                ...
                if parentIsMutex && mtxConstraint <= 0 {
                    ...
                    if childResult is Entitlement {
                        ...
                        let newPurchaseOptionResult = PurchaseOption(other:newResult)
                    ...
                    _ = calculateFeatursTree(parent:parent.children[orderedItem.origIndex],results:&results,mutexConstraint:-1,errorInfo:&errorInfo)
                    ...
                    results.addFeature(parentName:parentName,newFeature:childResult)
                    _ = calculateFeatursTree(parent:parent.children[orderedItem.origIndex],results:&results,mutexConstraint:mtxConstraint,errorInfo:&errorInfo)
                }
            } else {
                results.addFeature(parentName:parentName,newFeature:childResult)
                _ = calculateFeatursTree(parent:parent.children[orderedItem.origIndex],results:&results,mutexConstraint:mtxConstraint,errorInfo:&errorInfo)
    ...
    func calculateFeatureConfigurationAndOrder(feature:Feature,errorInfo:inout [JSErrorInfo]) -> Feature {
        ...
        let result = getConfiguarationResults(feature:feature,errorInfo:&errorInfo)
    ...
    func getOrderResults (feature:Feature,errorInfo:inout [JSErrorInfo]) -> Feature {
        ...
        if let childrenOrder = calculateOrderingRules(feature:feature,firedOrderingNames:&firedOrderingNames) {
    ...
    func calculateConfigurationFeature(parent:Feature,configuarationArr:[Feature],mutexConstraint:Int,firedConfigNames:inout [String:Bool],configNameThatTurnOffFeature:inout String) -> configNodeResult {
        ...
        for configRule in configuarationArr {
            ...
            if evaluateConfigurationItem(configRule:configRule,out:&oneConfigRule) { //rule true
                ...
                guard let oneConfigRuleChildrean:[String:AnyObject] = configuarationChildResult.cofiguaration else {
    ...
    func calculateExperiments(experimentsFeatures:FeaturesCache,errorInfo:inout [JSErrorInfo]) -> ExperimentsResults {
        ...
        guard let root = experimentsFeatures.getRoot(), let experimentsPercentage = Airlock.sharedInstance.percentageExperimentsMgr else {
            return experimentsResults
        }
        
        percentageMgr = experimentsPercentage
        for child in root.children {
            if child.type == .MUTUAL_EXCLUSION_GROUP {
                let mxFeature = Feature(type:child.type,uniqueId:child.uniqueId,name:child.name,source:child.source)
        ...
        for experimentFeature in parent.children {
            ...
            if experimentResultFeature.isOn() {
                if calculateVariantsTree(parentExperiment:experimentFeature,experimentsResults:&experimentsResults,errorInfo:&errorInfo) {
                    ...
                    let expFeature = experimentsResults.resultsFeatures.getFeature(featureName:experimentResultFeature.name)
    ...
    func calculateExperimentAndVarient(feature:Feature,errorInfo:inout [JSErrorInfo]) -> Feature {
        ...
        case .RULE_ERROR:
            let traceStr = "Rule Error: \(jsInvoker.getErrorMessage())"
    ...
    func calaculatePremiumData(feature:Feature,result:inout Feature,errorInfo:inout [JSErrorInfo]) {
        ...
        let resultPremiumData = FeaturePremiumData()
    ...
    func calculateEntitlementPurchaseOptions(entitlement:Entitlement,resultEntitlement:inout Entitlement,errorInfo:inout [JSErrorInfo]) {
        ...
        let rootResult = PurchaseOption(type:Type.ROOT,uniqueId:"",name:Feature.ROOT_NAME,source:.SERVER)
        ...
        var resultsProtocol = resultEntitlement as FeaturesMangement
    ...
    func calculatePurchaseOption(feature:Feature,result:inout Feature,errorInfo:inout [JSErrorInfo]) {
        ...
        let resultPurchaseOption = PurchaseOption(other:result)
