...
class FeaturesCache : NSObject,NSCoding,FeaturesMangement {
    ...
    func addFeature(parentName:String?,newFeature:Feature) {
        if let parentName = parentName {
            if let parent = featuresDict[parentName.lowercased()] {
    ...
    func buildExperiments(inputExperiments:[String:AnyObject],experimentsAnalytics:inout [String:[String:AnyObject]]) {
        let rootFeature:Feature = Feature(type:Type.ROOT,uniqueId:"",name:Feature.ROOT_NAME)
        ...
        let experimentsMX:Feature = Feature(type:.MUTUAL_EXCLUSION_GROUP,uniqueId:EXPERIMENTS_MX_UID,name:"\(MUTUAL_EXCLUSION_PREFIX).\(EXPERIMENTS_MX_NAME)")
    ...
    static func validateMXCount(mxFeature:Feature,result:inout [String:Feature]) {
        ...
        for fName in mxFeature.childrenNames {
            if let child:Feature = result[fName.lowercased()] {
                if child.isOn() {
                    maxOn = maxOn - 1
                    if maxOn < 0 {
    ...
    func buildConfigurations() {
        ...
        for (_,feature) in self.featuresDict {
    ...
    func mergeSendToAnalytics(data: [String]) {
        let featuresCopy = self.featuresDict
        let configurationsCopy = self.configurationsDict
        for (name) in data {
    ...
    func setFeaturesRefernces(resultDict:inout [String:Feature]) {
        ...
        for (_,feature) in resultDict {
            ...
            for childName in feature.childrenNames {
                if let childFeature = resultDict[childName.lowercased()] {
    ...
    static func buildExperiment(experimentDict:[String:AnyObject]) -> Feature {
        ...
        let experimentFeature = Feature(type:.EXPERIMENT,uniqueId:uID,name:fullName)
    ...
    static func buildVariant(variantDict:[String:AnyObject]) -> Feature {
        ...
        let variantFeature = Feature(type:.VARIANT,uniqueId:uID,name:fullName)
    ...
    static func buildEntitlement(entitlementDict:[String:AnyObject],runTime:Bool) -> Entitlement? {
        ...
        guard let retEntitlement = buildFeature(featureDict:entitlementDict,runTime:runTime) as? Entitlement else {
            return nil
        }
    ...
    static func buildPurchaseOption(purchaseOptionDict:[String:AnyObject],runTime:Bool) -> PurchaseOption? {
        ...
        guard let retPurchaseOption = buildFeature(featureDict:purchaseOptionDict,runTime:runTime) as? PurchaseOption else {
            return nil
        }
        ...
        for storeProductIdDict in storeProductIdsDict {
            let storeType = storeProductIdDict[STORE_TYPE_PROP] ?? ""
            let productId = storeProductIdDict[PRODUCT_ID_PROP] ?? ""
    ...
    static func buildFeature(featureDict:[String:AnyObject],runTime:Bool) -> Feature {
        ...
        let branchStatus:BranchStatus = BranchStatus(rawValue:branchStatusStr) ?? .None
    ...
    static func mergeFeature(outParentNameLowercased:String,fromChild:Feature,to:FeaturesCache,out:inout FeaturesCache) {
        ...
        let outFeature:Feature = doMergeFeatures(to:toChild,from:fromChild)
