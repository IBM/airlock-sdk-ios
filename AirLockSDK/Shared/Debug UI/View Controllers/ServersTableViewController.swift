...
class ServersTableViewController: UITableViewController {
    ...
    @IBAction func clearOverridingServer(_ sender: Any) {
        ...
        refreshAlert.addAction(UIAlertAction(title: "Yes", style: .default, handler: { (action: UIAlertAction!) in
            ...
            Airlock.sharedInstance.pullFeatures(onCompletion: { success, error in
                ...
                do {
                    try _ = Airlock.sharedInstance.calculateFeatures(deviceContextJSON: context)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let currServerName = self.serversList[indexPath.row]
        ...
        guard let b = ServersTableViewController.bundle else {
            return cell!
        }
    ...
    private func loadServersList() {
        ...
        guard let _:URL = Airlock.sharedInstance.getServerBaseURL(originalServer: true) else {
        ...
        Airlock.sharedInstance.serversMgr.retrieveServers(onCompletion: { servers, defualtServer, error in
            ...
            guard error == nil else {
                DispatchQueue.main.async {
                    self.activityIndicator.stopAnimating()
                    self.showAlert(title: "Airlock", message: "Failed to retrieve servers. error:\(String(describing: error))")
                }
                return
            }
