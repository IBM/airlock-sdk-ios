        ...
        if indexPath.section == 0 {
            ...
            if indexPath.row == 0 {
                if let debugBannersCell = cell as? AirlyticsDebugBannersTableViewCell {
                    debugBannersCell.debugBannersSwitch.isOn = UserDefaults.standard.bool(forKey: Airlytics.Constants.DEBUG_BANNERS_KEY)
                }
            } else {
                if let debugLogCell = cell as? AirlyticsDebugLogTableViewCell {
                    debugLogCell.debugLogSwitch.isOn = UserDefaults.standard.bool(forKey: Airlytics.Constants.DEBUG_LOG_KEY)                }
            }
