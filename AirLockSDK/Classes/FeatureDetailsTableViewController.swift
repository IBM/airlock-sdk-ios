    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showConfigSegue" {
            ...
            if let runTimeFeature = getRunTimeFeature() {
                ...
                for (n,b) in data!.feature.firedConfigNames {
                    text += "⇨ \(n)\n"
                }
            }
            ...
            if data!.feature.firedOrderConfigNames.count == 0 {
                ...
                if let runTimeFeature = getRunTimeFeature() {
                    ...
                    var j = 1
                    for (i, or) in orderRulesArr.enumerated() {
                        let name = or.getName()
                    ...
                    for (offset: i, element: (key: n,value: b)) in data!.feature.firedConfigNames.enumerated() {
                        text += "\(i + 1).\t" + n + "\n"
                    }
                }
            }
